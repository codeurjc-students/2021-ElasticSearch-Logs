...
import { COLUMN_DEFS } from '../../log/components/table/components/config/table.config';
...
export class DataProcessor {
    ...
    private replaceText(text: string, stringToHighlight: string): string {
        ...
        if (upperText.includes(upperString)) {
            const startWord = upperText.indexOf(upperString);
            const word = text.substring(
                startWord,
                startWord + stringToHighlight.length
            );
    ...
    public buildFormControl(columns: any[], initParam: any): any {
        return columns.reduce(
            (acc, column) =>
                Object.assign(acc, {
                    [column.field]: new FormControl(initParam),
                }),
            {}
        );
    }
